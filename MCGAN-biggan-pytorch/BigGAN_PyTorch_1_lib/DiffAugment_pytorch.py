...
def DiffAugment(x, policy='', channels_first=True,x_mc=None,mc_size=10):
'''
...
'''
    if policy:
        ...
        #reshape x_mc to (MC,B,C,H,W)
            x_mc=x_m.reshape(mc_size,x.shape[0],x.shape[1],x.shape[2],x.shape[3])
...
def rand_brightness(x,x_mc=None):
...
def rand_saturation(x,x_mc=None):
