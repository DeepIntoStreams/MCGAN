...
def calc_metric(metric, **kwargs): # See metric_utils.MetricOptions for the full list of arguments.
    ...
    for key, value in list(results.items()):
        if opts.num_gpus > 1:
            ...
            value = float(value.cpu())  #modify this part 
        results[key] = value
...
@register_metric
def intrafid50k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    intrafid_mean,intrafid = frechet_inception_distance.compute_intrafid(opts, max_real=None, num_gen=50000)
    return dict(intrafid50k_full=intrafid_mean, fid_class=intrafid)
