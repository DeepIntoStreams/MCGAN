...
), and put it under `./data`. Then run the following line for 4 gpus:
```bash
...
### Imagenet64
Download Imagenet64 dataset from [Image-net](https://image-net.org/download-images.php), and put it under `./data`. Then run the following line for 4 gpus:
```bash
...
### LSUN bedroom 256:
Download FFHQ256 dataset by following instructions in [LSUN](https://github.com/fyu/lsun), and put it under `./data`. Then run the following line for 4 gpus:
```bash
