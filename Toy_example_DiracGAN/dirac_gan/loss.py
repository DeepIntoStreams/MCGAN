...
class WassersteinGANLossDiscriminator(nn.Module):
    ...
    def forward(self, discriminator_prediction_real: torch.Tensor,
        ...
        return - discriminator_prediction_real.mean() \
               + discriminator_prediction_fake.mean() +5* (discriminator_prediction_real.pow(2)+discriminator_prediction_fake.pow(2)).mean()
...
class LSGANLossGenerator(nn.Module):
    ...
    def forward(self, discriminator_prediction_fake: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        """
        return  0.5 * (discriminator_prediction_fake+0.5 - 1.).pow(2).mean()
...
class LSGANLossDiscriminator(nn.Module):
    ...
    def forward(self, discriminator_prediction_real: torch.Tensor,
        ...
        """
        return 0.5 * (( discriminator_prediction_real+0.5 - 1.).pow(2).mean()
                      + (discriminator_prediction_fake+0.5).pow(2).mean())
...
class CLC(nn.Module):
    ...
    def forward(self, discriminator_prediction_real: torch.Tensor,
        ...
        """
        regularization_loss = discriminator_prediction_real.pow(2).mean() \
                              + discriminator_prediction_fake.pow(2).mean()
        return HYPERPARAMETERS["clc"] * regularization_loss
...
class DRAGANLossDiscriminator(nn.Module):
    ...
    def forward(self, discriminator_prediction_real: torch.Tensor,
        ...
        loss = loss + HYPERPARAMETERS["dra_w"] * (grad_real.norm(dim=1) - 1.).pow(2).mean()
        return loss.mean()
    
class MCGANLossGenerator(GANLossGenerator):
    """
    This class implements the generator loss proposed in:
    https://arxiv.org/pdf/1705.07215.pdf
    """
    ...
    def __init__(self) -> None:
        ...
        """
        # Call super constructor
        super(MCGANLossGenerator, self).__init__()

    def forward(self,discriminator_prediction_fake: torch.Tensor) -> torch.Tensor:
        """
        Forward pass.
        :param discriminator_prediction_fake: (torch.Tensor) Raw discriminator predictions for fake samples
        :return: (torch.Tensor) Generator LSGAN loss
...
class MCGANLossDiscriminator(nn.Module):
    """
    This class implements the generator loss proposed in:
    https://arxiv.org/pdf/1705.07215.pdf
    """
    ...
    def __init__(self) -> None:
        ...
        """
        # Call super constructor
        super(MCGANLossDiscriminator, self).__init__()
    ...
    def forward(self, discriminator_prediction_real: torch.Tensor,
        ...
        return - torch.minimum(torch.tensor(0., dtype=torch.float, device=discriminator_prediction_real.device),
               ...
               - torch.minimum(torch.tensor(0., dtype=torch.float, device=discriminator_prediction_fake.device),
                               - discriminator_prediction_fake - 1.).mean()
    # 0.5 * (( -discriminator_prediction_real-0.5).pow(2).mean()
               #       + (discriminator_prediction_fake+0.5).pow(2).mean())
               #F.softplus(- discriminator_prediction_real).mean() \
               #+ F.softplus(discriminator_prediction_fake).mean()
               ##- torch.minimum(torch.tensor(0., dtype=torch.float, device=discriminator_prediction_real.device),
               #                discriminator_prediction_real - 1.).mean() \
               #- torch.minimum(torch.tensor(0., dtype=torch.float, device=discriminator_prediction_fake.device),
               #                - discriminator_prediction_fake - 1.).mean()
