...
import animal_hash
import tensorflow as tf #############update set_rng function for TF tp fix seed
import torch
...
def get_data_loaders(dataset, data_root=None, augment=False, batch_size=64, 
  ...
  if 'hdf5' in dataset:
    ...
    train_transform = transforms.Compose(train_transform + [
                     transforms.ToTensor(),
                     transforms.Normalize(norm_mean, norm_std)])#normalize image value to range [-1,1]
...
def interp_sheet(G, num_per_sheet, num_midpoints, num_classes, parallel,
  ...
  if fix_y: # If fix y, only sample 1 z per row
    ...
    ys = ys.repeat(1, num_midpoints + 2, 1).view(num_per_sheet * (num_midpoints + 2), -1).to(zs.device)
  else:
    ys = interp(G.shared(sample_1hot(num_per_sheet, num_classes)).view(num_per_sheet, 1, -1),
                ...
                num_midpoints).view(num_per_sheet * (num_midpoints + 2), -1).to(zs.device)
