...
def setup_training_loop_kwargs(
    ...
    mc_size    = None, # Size used in MonteCarlo method when training generator.
):
    ...
    args.training_set_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data, use_labels=True, max_size=None, xflip=False)# xflip=True
    ...
    cfg_specs = {
        ...
        'cifar':     dict(ref_gpus=4,  kimg=100000, mb=64, mbstd=16, fmaps=1,   lrate=0.0025, gamma=0.01, ema=500, ramp=0.05, map=2),# mb= 64 mbstd= 32
    }
    ...
    args.G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=0.0025, betas=[0,0.99], eps=1e-8, weight_decay=0)# add weight_decay=weight_decay
    args.D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=0.0025, betas=[0,0.99], eps=1e-8, weight_decay=0)# addd 
    args.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.StyleGAN2Loss', r1_gamma=spec.gamma)
    
    args.loss_kwargs.mc_size = 1 if mc_size== None else mc_size
...
@click.option('--mc_size', help='Sample size of Monte-Carlo method [default: 1]', type=int, metavar='INT')
...
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    print(f'Monte-Carlo size:   {args.loss_kwargs.mc_size}')
    print()
