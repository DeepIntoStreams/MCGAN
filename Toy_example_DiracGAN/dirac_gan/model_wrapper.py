...
from .loss import WassersteinGANLossGPDiscriminator, R1, DRAGANLossDiscriminator, RLC,CLC
...
class ModelWrapper(object):
    ...
    def generate_trajectory(self, steps: int = 10, instance_noise: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for generator_parameter, discriminator_parameter in zip(generator_parameters, discriminator_parameters):
            ...
            if self.regularization_loss is not None:
                ...
                elif isinstance(self.regularization_loss, RLC) or isinstance(self.regularization_loss, CLC):
                    discriminator_loss: torch.Tensor = self.regularization_loss(
    ...
    def train(self, instance_noise: bool = True) -> torch.Tensor:
        ...
        for iteration in range(HYPERPARAMETERS["training_iterations"]):
            ...
            self.discriminator_optimizer.zero_grad()
            # Make fake prediction
            fake_prediction: torch.Tensor = self.discriminator(self.generator(get_noise(HYPERPARAMETERS["batch_size"]*2) if self.MC else get_noise(HYPERPARAMETERS["batch_size"])))
            # Make real prediction
            if self.MC:
                real_samples: torch.Tensor = torch.zeros(HYPERPARAMETERS["batch_size"], 1)
                real_samples.requires_grad = True
                real_prediction: torch.Tensor = self.discriminator(real_samples)
                generator_loss: torch.Tensor =(1-self.gamma)*(real_prediction-fake_prediction.reshape(2,HYPERPARAMETERS["batch_size"],1).mean(0)).pow(2).mean()+self.gamma*self.generator_loss_function(fake_prediction)#
            # Compute generator
            else:                         
                generator_loss: torch.Tensor = self.generator_loss_function(fake_prediction)
            # Compute gradients
